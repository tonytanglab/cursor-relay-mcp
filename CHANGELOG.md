...
### 🕒 2026-09-25 10:41 | [FIX] 本地持久化失败不再中断 Cursor 事件观察

- **问题与证据**：两条真实运行在 Relay 的 `relay-state.json` 写入失败后退出事件订阅，而 SDK SQLite 仍追加事件；旧日志丢失系统错误码，不能断言是 Cursor 模型退出、Codex 主程序故障或某个具体 Windows 错误。状态文件约66 MB，旧插件仍逐事件读写整份历史。
- **变更内容**：隔离本地持久化与 SDK 流错误，同一运行保留当前事件并按100 ms至5 s退避恢复，不重提模型任务；终态持久化完成前不报告完成。查询、等待和面板暴露 `persistence` 与 `needsAttention`，保留错误阶段、系统码和提交状态。已提交但锁清理失败时仅恢复同一锁，不重跑mutator。
- **性能与兼容**：去掉事件追加前重复全量读取及读后重复拷贝，JSON紧凑序列化；保留旧schema、完整历史、锁和原子替换。没有升级 `@cursor/sdk 1.0.30`，不改凭据、不取消或续提已有运行、不修改PackCAD。
- **部署**：发布缓存版本为 `0.1.1+codex.20260925023100`；其它电脑升级手册要求构建、重装，并在新连接核对 `doctor.capabilities.localPersistenceRecovery=true`。Git已更新不等于本机安装已更新，旧运行不热切换。
- **验证**：format:check、lint、typecheck、102/102测试、build、test:mcp（14工具且核对新恢复能力）、check:package及插件/Skill校验通过。故障注入覆盖两路共享状态、事件/起跑登记/终态保存失败、锁清理不重放、前置目录失败与真实截止时间。首轮并发启动测试暴露登记时序及清理竞态，修复后全量复验通过。未新增真实付费模型运行，未把旧缓存连接的响应当成新缓存生效证据。
- **边界与回滚**：本地磁盘长时间不可写仍需处理，恢复不会复活退出的执行器；SDK事件缓冲溢出须按真实流错误披露，最终仍读取真实结果。回退代码并重建重装可回滚，保留状态和历史，不手改运行完成状态。
